...
int Setup_GPIO(void){
	...
	if(request_mem_region(GPIO_PA_BASE, 0x1c, DEV_NAME) == NULL){
		return -1;} //error
	if(request_mem_region(GPIO_PA_BASE, 0x11c, DEV_NAME) == NULL){
		return -2;} //error

	mem_reg_GPIO_PORT_C = ioremap_nocache(GPIO_PC_BASE, 0x020);
	...
	/*********configure interrupt*************/
	iowrite32(0x22222222, mem_reg_GPIO_INT);	  //GPIO_EXTIPSELL
	iowrite32(0xff,		  mem_reg_GPIO_INT+ 0x0c);//GPIO_EXTIFALL
	iowrite32(0xff,		  mem_reg_GPIO_INT+ 0x1c);//GPIO_IEN
...
static irq_handler_t Button_handler(int irw, void *dev_id, struct pt_regs *regs) {
...
static int __init template_init(void)
{
	...
	switch(Setup_GPIO()){
		case 0: 
		case -1:
			printk("can't get access to GPIO_PA register");
	...
	if(alloc_chrdev_region(&DEV_ID, 1, 0, DEV_NAME) < 0) goto fail_alloc;
